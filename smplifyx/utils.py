...
def get_all_paths(root: str) -> List[str]:
    return [os.path.join(root, f) for f in os.listdir(root) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]


def get_all_ehf_paths(root: str) -> List[str]:
    return [os.path.join(root, f) for f in os.listdir(root) if f.lower().endswith('.jpg')]


def get_all_hbw_paths(root: str) -> List[str]:
    paths = []
    root = os.path.join(root, "images/val")
    for folder in os.listdir(root):
        root_folder = os.path.join(root, folder)
        paths.extend([os.path.join(root_folder, "Photos_Lab", f) for f in os.listdir(os.path.join(root_folder, "Photos_Lab"))])
        paths.extend([os.path.join(root_folder, "Pictures_in_the_Wild", f) for f in os.listdir(os.path.join(root_folder, "Pictures_in_the_Wild"))])
    return paths


def get_all_ssp3d_paths(root: str) -> List[str]:
